...
import { API_ENDPOINTS, getHostUrl } from './config';
...
export const fetchAlertsApi = async (token?: string | null): Promise<AlertItem[]> => {
  const response = await fetchWithAuth(API_ENDPOINTS.ALERTS.BASE, {
    method: 'GET',
  });
  ...
  if (!response.ok) {
    const err: any = new Error(json.message || json.error || `Failed to fetch alerts: ${response.status}`);
...
export const updateAlertStatusApi = async (
  ...
  try {
    ...
    const response = await fetch(API_ENDPOINTS.ALERTS.BY_ID(alertId), {
      method: 'PUT',
      headers,
